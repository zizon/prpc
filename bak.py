...
class Scheduler(object):
	...
	def do(self):
		...
		applications.sort(key=lambda x:x['resouces']['cpu'])
		...
		# find less greedy one	
		applications.sort(key=lambda x:x['greedy']['memory'])
		...
		movable_tasks = collections.deque(
			filter(lambda x:x['queue'] in overloaded,applications)
		)
		
		for candidate in filter(lambda x:x['usage'] < 1,queues):
			for choice in xrange(len(movable_tasks)):	
				task = logging.info('for choie:%s queue:%s total:%s' % (choice,candidate,len(movable_tasks)))
				task = movable_tasks.popleft()
				logging.info('select application_%s_%s move to %s' % (
					task['id']['cluster_timestamp'],
					task['id']['id'],
					candidate['name'],
				))
				if not self._move(task,candidate):
